	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			for(int s=0; s<dealDate.size(); s++) {
				...
				// 전월세 보증금 구하기
				sql = "select sum(replace(deposit,',','')) sum_rent_amt from apt_rent_"+table_nm+" where sgg_cd = ? and deal_year = ? and deal_month = ?";
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, guCode);
				pstmt.setString(2, dealDate.get(s).substring(0,4));
				pstmt.setInt(3, Integer.parseInt(dealDate.get(s).substring(4,6)));
			...
			for(int s=0; s<dealDate.size(); s++) {
				...
				// 월별 전월세 거래량
				sql = "select count(*) from apt_rent_"+table_nm+" where sgg_cd = ? and deal_year = ? and deal_month = ?";
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, guCode);
				pstmt.setString(2, dealDate.get(s).substring(0,4));
				pstmt.setInt(3, Integer.parseInt(dealDate.get(s).substring(4,6)));
