 ...
 */
@WebServlet("/eupDealAmount")
public class eupDealAmount extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public eupDealAmount() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			// 기간구하기
			String sql = "select deal_year , deal_month from apt_"+table_nm+" where sgg_cd = ? and umd_cd like ? group by deal_year , deal_month order by cast(deal_year as unsigned) asc, cast(deal_month as unsigned) asc";
			//System.out.println(table_nm+", "+dongCode);
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, guCode);
			pstmt.setString(2, dongCode.substring(0,2)+"%");
			...
			for(int s=0; s<dealDate.size(); s++) {
				...
				sql = "select count(*) from apt_"+table_nm+" where sgg_cd = ? and umd_cd like ? and deal_year = ? and deal_month = ?";
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, guCode);
				pstmt.setString(2, dongCode.substring(0,2)+"%");
				pstmt.setString(3, dealDate.get(s).substring(0,4));
